...
#include <stdlib.h>
#include <unistd.h>
#include <stdint.h>  // Biblioteca necessária para intptr_t
...
void* gerador_incendios_thread(void* args);
...
int main() {
    ...
    pthread_create(&gerador, NULL, gerador_incendios_thread, NULL);

    // Exibir a floresta periodicamente
    while (1) {
        imprimir_floresta();  // Exibe o estado atual da floresta
